...
_LIMIT_PERC_OFFSET_V = [10, 15, 15, 20]  # 55, 105, 135 km/h
_LIMIT_PERC_OFFSET_BP = [60, 61, 99, 100] # 50, 100, 130 km/h
...
class SpeedLimitController():
  ...
  def update(self, enabled, v_ego, a_ego, sm, v_cruise_setpoint, events=Events()):
    ...
    self._op_enabled = sm['controlsState'].active and _car_state.cruiseState.enabled and \
                       not _car_state.brakePressed and \
                       not (events.any(ET.OVERRIDE_LONGITUDINAL) and self._disengage_on_accelerator) and \
                       not events.any(ET.NO_ENTRY)
    self._v_ego = v_ego
    self._a_ego = a_ego
