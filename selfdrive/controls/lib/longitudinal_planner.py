...
def limit_accel_in_turns(v_ego, angle_steers, a_target, live_params, VM):
  ...
  a_y = v_ego * omega
  a_x_allowed = math.sqrt(max(a_total_max ** 2 - a_y ** 2, 0.)) * 1.25
...
class LongitudinalPlanner:
  ...
  def update(self, sm):
    ...
    self.mpc.update(sm['carState'], sm['radarState'], v_cruise_sol, x, v, a, j, personality=self.personality, personality_mode=self.gap_adjust_cruise)
  ...
  def cruise_solutions(self, enabled, v_ego, a_ego, v_cruise, sm):
    ...
    speed_limit_controller_exp_mode = self.speed_limit_controller._resolver.force_exp_mode
    self.turn_speed_controller.update(enabled, v_ego, a_ego, sm)
    turn_speed_controller_exp_mode = self.turn_speed_controller.state in (TurnSpeedControlState.adapting, TurnSpeedControlState.active) \
                                     and self.turn_speed_controller.a_target - a_ego < -0.3 \
                                     and self.turn_speed_controller.distance > 0
                                  
    self.experimental_controller.update(enabled, v_ego, sm, turn_speed_controller_exp_mode, speed_limit_controller_exp_mode)

    # Pick solution with the lowest velocity target.
    a_solutions = {'cruise': float("inf")}
    v_solutions = {'cruise': v_cruise}

    if self.speed_limit_controller.is_active: # and not self.turn_speed_controller.is_active:
